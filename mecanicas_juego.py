...
#def elegir ficha (recibe una posicion devuelve una letra)
        ...
        #De ser válida:
#girar ficha (recibe una posicion retorna el tablero)
        tablero , par_igual = girar_ficha (opcion_1,0,tablero)#Función del archivo

        mostrar_tablero(tablero)
        print("Elija otra ficha")
...
#renovar tablero(recibe las dos posiciones y el tablero y entrega un nuevo tablero )
        if par_igual : 
        ...
        #Si todos los pares de fichas son adivinados
        if pares == int(len(tablero)/2) :
                print ("\033[0;31m"+"Fin del juego"+"\033[0m")
                completo = True

    turno=[completo , tablero , jugadores , pares]
    
    return turno

#Luciano: La función controla que el numero corresponda a una posicion valida del tablero.
...
"""
Luciano: La función:
*Recibe los valores seleccionados con las relacionadas en el tablero
*Compara las fichas y determina si el jugador continúa y recibe puntos
"""
def girar_ficha (primer_numero,segundo_numero, tablero, reset=False):
    #Se ajustan los valores seleccionados al rango de la lista
    primer_numero += -1
    segundo_numero += -1
    resultado=[]
    par_igual = False
    ...
    resultado=[tablero, par_igual]
            
    return resultado

#Luciano: cronometro: retorna el tiempo que duro la partida, restando el tiempo de inicio de partida (que recibe como parametro) al tiempo actual.
